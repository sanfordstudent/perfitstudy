# Description ###################################################################################
# This file creates a series of simulated item response datasets, based on the methodology of 
# Karabatsos (2003) and Sinharay (2017). First, I create 60 datasets based on a 3x4x5 fully crossed
# design, with three test lengths, four proportions of aberranct responding and five types of aberrant
# responding. The number of examinees is always 10,000. In contrast to the studies on which this
# is based, I assume a standard normal distribution of examinee abilities.
#################################################################################################
set.seed(102856)
library(MESS)
library(tidyverse)
...
# truethetas <- rnorm(10000) %>% sort() %>% data.frame(theta = .)
# mean(truethetas$theta)
# sd(truethetas$theta)
# 
# ggplot(truethetas, aes(x = theta, y = ..density..)) + geom_density() + geom_histogram(binwidth = 0.1, fill = NA, color = "black")

# generate a truly normal distribution of thetas by sampling evenly spaced values from every quantile; probably don't 
# actually want to do this
# truethetas <- data.frame(theta = qnorm(seq(1/(n+1),n/(n+1),length=n)))
# mean(truethetas$theta)
# sd(truethetas$theta)
# ggplot(truethetas, aes(x = theta, y = ..density..)) + geom_density() + geom_histogram(binwidth = 0.1, fill = NA, color = "black")
...
for(i in 1:60){
  ...
  # assign true thetas by a normal distribution
  examinees$theta <- rnorm(n)
  ...
  if(type=="ch"){
    ...
    examinees[sample(1:round(n*pnorm(-0.5)), amt, replace = FALSE),]$aberrant <- TRUE
    examinees[which(examinees$aberrant==TRUE),]$type <- "cheating"
    
    # select a random subset of 18% of harder 50% of items as "compromised"
    compromised <- sample(as.integer(length/2):length, size = as.integer(0.18*length), replace = FALSE)
    ...
    # select amt/2 creative from the examinees with ability above about 0.5
    crs <- sample(round(n*pnorm(0.5)):n, amt/2, replace = FALSE)
    examinees[crs,]$aberrant <- TRUE
    examinees[which(examinees$aberrant==TRUE),]$type <- "creative"
    
    # select amt/2 cheating from the examinees with ability below -0.5
    examinees[sample(round(n*pnorm(-0.5)):n, amt/2, replace = FALSE),]$aberrant <- TRUE
    examinees[which(examinees$aberrant==TRUE & examinees$type != "creative"),]$type <- "cheating"
    # select a random subset of 18% of items as "compromised"
    compromised <- sample(as.integer(length/2):length, size = as.integer(0.18*length), replace = FALSE)
    ...
    # select amt/2 each guessing and cheating from the examinees with ability below -0.5
    examinees[sample(1:round(n*pnorm(-0.5)), amt, replace = FALSE),]$aberrant <- TRUE
    examinees[which(examinees$aberrant==TRUE),]$type <- "guessing"
    examinees[sample(which(examinees$aberrant==TRUE), size = amt/2, replace = FALSE),]$type <- "cheating"
    
    # select a random subset of 18% of items as "compromised"
    compromised <- sample(as.integer(length/2):length, size = as.integer(0.18*length), replace = FALSE)
    
    # now, simulate guessing and cheating
    # then simulate the normal respondents via the Rasch model
    for(k in 1:length){
      item <- paste("i", k, sep="")
      examinees[[item]] <- apply(
    ...
    # select amt/2 careless from the examinees with ability above about 0.5
    examinees[sample(round(n*pnorm(0.5)):n, amt/2, replace = FALSE),]$aberrant <- TRUE
    examinees[which(examinees$aberrant==TRUE),]$type <- "careless"
    
    # select amt/2 cheating from the examinees with ability below -0.5
    examinees[sample(round(n*pnorm(-0.5)):n, amt/2, replace = FALSE),]$aberrant <- TRUE
    examinees[which(examinees$aberrant==TRUE & examinees$type != "careless"),]$type <- "cheating"
    
    # select a random subset of 18% of items as "compromised"
    compromised <- sample(as.integer(length/2):length, size = as.integer(0.18*length), replace = FALSE)
    
    # now, simulate  answering carelessly/cheating
    # then simulate the normal respondents via the Rasch model
    for(k in 1:length){
      item <- paste("i", k, sep="")
      examinees[[item]] <- apply(
    ...
    # select amt/2 random and cheating from the examinees with ability below -0.5
    examinees[sample(1:round(n*pnorm(-0.5)), amt, replace = FALSE),]$aberrant <- TRUE
    examinees[which(examinees$aberrant==TRUE),]$type <- "random"
    examinees[sample(which(examinees$aberrant==TRUE), size = amt/2, replace = FALSE),]$type <- "cheating"
    
    # select a random subset of 18% of hard items as "compromised"
    compromised <- sample(as.integer(length/2):length, size = as.integer(0.18*length), replace = FALSE)
    ...
    for(k in 1:length){
      ...
      examinees[[item]] <- apply(
        ...
        function(drow){
          switch(
            drow["type"],
            "random" = rbernoulli(1, 0.25) %>% as.numeric(),
            "cheating" = ifelse(
              k %in% compromised,
              1,
              rbernoulli(
  ...
  good=stats[which(examinees$type != "cheating"),]#PFSs for the non-cheating examinees
  bad=stats[which(examinees$type == "cheating"),]#PFSs for the cheating examinees
  ...
  SEs.ch=rbind(SEs.ch,ROCOut[2,])
  Output.ch=rbind(Output.ch,c(length,as.character(type),100*condition$amount,Areas))
...
write.csv(Output.ch, "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/output_ch.csv")
write.csv(Output.all, "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/output_all.csv")

# mean AUROC over all conditions
out.ch <- read.csv("/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/output_ch.csv")[,2:12]
names(out.ch) <- c("length", "type", "percent", "lz*", "ht", "u3", "eci4z*", "sha1/2", "sha1", "shb2", "shb3")

out.all <- read.csv("/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/output_all.csv")[,2:12]
names(out.all) <- c("length", "type", "percent", "lz*", "ht", "u3", "eci4z*", "sha1/2", "sha1", "shb2", "shb3")

out.ch.summary <- out.ch %>% pivot_longer(
  names_to = "stat",
  values_to = "AUROC",
  cols = c(
...
out.ch.summary.length <- out.ch %>% pivot_longer(
  names_to = "stat",
  values_to = "AUROC",
  cols = c(
    `lz*`, ht, u3, `eci4z*`, `sha1/2`, sha1, shb2, shb3
...
out.ch.summary.type <- out.ch %>% pivot_longer(
  names_to = "stat",
  values_to = "AUROC",
  cols = c(
    `lz*`, ht, u3, `eci4z*`, `sha1/2`, sha1, shb2, shb3
...
out.ch.summary.pct <- out.ch %>% pivot_longer(
  names_to = "stat",
  values_to = "AUROC",
  cols = c(
    `lz*`, ht, u3, `eci4z*`, `sha1/2`, sha1, shb2, shb3
...
## Create ROC curves to investigate low performance for SH's
# example
# first attempt, sha1/2
# the curves look normal, just bad
df <- data.frame(
  predictions = stats[,7], # switch to 2 for ht
  labels = (examinees$aberrant == FALSE) %>% as.numeric()
)
...
for (stat in unique(dsl.dat$stat)){
  out.ch.overall.dsl=c(
    out.ch.overall.dsl,
    rma(
      yi = as.numeric(dsl.dat[which(dsl.dat$stat==stat),]$AUC),
      sei = as.numeric(dsl.dat[which(dsl.dat$stat==stat),]$se),
      # data = data.frame(yi = Areas[,i], sei = SEs.ch[,i]),
      method="DL"
    )$beta
  )
}

out.ch.overall.dsl <- data.frame(out.ch.overall.dsl %>% t())

names(out.ch.overall.dsl) <- c("lz*", "ht", "u3", "eci4z*", "sha1/2", "sha1", "shb2", "shb3")

out.ch.overall.dsl %>% round(digits=2) %>% write.csv(
    "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/out_ch_overall.csv"
  )

## test length
out.ch.length.dsl <- NULL
Areas <- out.ch
Areas <- Areas %>% pivot_longer(names_to = "stat", values_to = "AUC", cols = c("lz*", "ht", "u3", "eci4z*", "sha1/2", "sha1", "shb2", "shb3"))
se.df <- data.frame(SEs.ch)
names(se.df) <- c("lz*", "ht", "u3", "eci4z*", "sha1/2", "sha1", "shb2", "shb3")
...
for(length in unique(dsl.dat$length)) {
  for (stat in unique(dsl.dat$stat)){
     ...
     out.ch.length.dsl <- rbind(out.ch.length.dsl, c(length, stat, auc))
  }
}

out.ch.length.dsl <- data.frame(out.ch.length.dsl)

names(out.ch.length.dsl) <- c("length", "stat", "auc")
out.ch.length.dsl %>% mutate(auc = round(as.numeric(as.character(auc)), digits = 2)) %>%
  pivot_wider(names_from = stat, values_from = auc) %>% write.csv(
    "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/out_ch_length.csv"
  )

## test type
out.ch.type.dsl <- NULL
Areas <- out.ch
Areas <- Areas %>% pivot_longer(names_to = "stat", values_to = "AUC", cols = c("lz*", "ht", "u3", "eci4z*", "sha1/2", "sha1", "shb2", "shb3"))
se.df <- data.frame(SEs.ch)
names(se.df) <- c("lz*", "ht", "u3", "eci4z*", "sha1/2", "sha1", "shb2", "shb3")
...
for(type in unique(dsl.dat$type)) {
  for (stat in unique(dsl.dat$stat)){
    auc <- rma(
      yi = as.numeric(dsl.dat[which(dsl.dat$stat==stat&dsl.dat$type==type),]$AUC),
      sei = as.numeric(dsl.dat[which(dsl.dat$stat==stat&dsl.dat$type==type),]$se),
      # data = data.frame(yi = Areas[,i], sei = SEs.ch[,i]),
      method="DL"
    )$beta
    out.ch.type.dsl <- rbind(out.ch.type.dsl, c(type, stat, auc))
  }
}

out.ch.type.dsl <- data.frame(out.ch.type.dsl)

names(out.ch.type.dsl) <- c("type", "stat", "auc")
out.ch.type.dsl %>% mutate(auc = round(as.numeric(as.character(auc)), digits = 2)) %>%
  pivot_wider(names_from = stat, values_from = auc) %>% write.csv(
    "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/out_ch_type.csv"
  )
...
for(pct in unique(dsl.dat$percent)) {
  for (stat in unique(dsl.dat$stat)){
    ...
    out.ch.pct.dsl <- rbind(out.ch.pct.dsl, c(pct, stat, auc))
  }
}

out.ch.pct.dsl <- data.frame(out.ch.pct.dsl)

names(out.ch.pct.dsl) <- c("pct", "stat", "auc")
out.ch.pct.dsl %>% mutate(auc = round(as.numeric(as.character(auc)), digits = 2)) %>%
  pivot_wider(names_from = stat, values_from = auc) %>% write.csv(
    "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/out_ch_pct.csv"
  )
...
out.all.overall.dsl %>% round(digits=2) %>% write.csv(
  "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/out_all_overall.csv"
)
...
out.all.length.dsl %>% mutate(auc = round(as.numeric(as.character(auc)), digits = 2)) %>%
  pivot_wider(names_from = stat, values_from = auc) %>% write.csv(
    "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/out_all_length.csv"
  )
...
out.all.type.dsl %>% mutate(auc = round(as.numeric(as.character(auc)), digits = 2)) %>%
  pivot_wider(names_from = stat, values_from = auc) %>% write.csv(
    "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/out_all_type.csv"
  )
...
out.all.pct.dsl %>% mutate(auc = round(as.numeric(as.character(auc)), digits = 2)) %>%
  pivot_wider(names_from = stat, values_from = auc) %>% write.csv(
    "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/out_all_pct.csv"
  )
...
write.csv(
  counts.bypct, 
  "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/counts_by_pct.csv"
)
...
write.csv(all, "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/overall_length_all.csv")


ch <- data.frame(rbind(Output.ch.17, Output.ch.33, Output.ch.65))
names(ch) <- c("lz*", "ht", "u3", "eci4z*", "sha1/2", "sha1", "shb2", "shb3")
write.csv(ch, "/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/overall_length_cheating.csv")
...
combined.17.summary %>% write.csv("/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/counts_17.csv")
combined.33.summary %>% write.csv("/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/counts_33.csv")
combined.65.summary %>% write.csv("/Users/sanfordstudent/Documents/Other Work/Person fit simulation project/tables/counts_65.csv")

# plots
# overall spread for new data and cheating
pdat <- out.ch %>% 
  pivot_longer(
    names_to="stat",
    values_to="value",
    cols=c("lz*","ht","u3","eci4z*", "sha1/2","sha1","shb2","shb3")
  ) %>% mutate(
      type = recode(
        type, 
        "ch" = "Cheating only", 
...
pdat %>%
  ggplot(aes(x = stat, y = value)) + geom_boxplot() + labs(x = "PFS", y = "AUROC") +
